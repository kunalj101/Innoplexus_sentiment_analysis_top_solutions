...
def get_drug_only_sentence(row):
    ...
    if len(idx) == 0:
        ...
        return re.sub(row["drug"], "drug A", unicodeToAscii(" ".join(sentences)), flags=re.IGNORECASE )  
    else:
        idx = idx[0]
    n = len(sentences)
    drug_sents = []
    if idx > 0:
        drug_sents.append(sentences[idx-1][-200:])
    ...
    return re.sub(row["drug"], "drug A", unicodeToAscii(" ".join(drug_sents)), flags=re.IGNORECASE)

def reframe_drug_question(drug):
    return "What do you think of drug A?"
...
if __name__ == "__main__":
    ...
    for fold in range(N_FOLDS):
        save_dir = Path(DATA_PATH) / "v3" / "fold_{}".format(fold)
